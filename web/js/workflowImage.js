...
class DebugLogger {
	...
	static log(message) {
		...
		console.log(logEntry); // Still log to console as backup
	}
...
class WorkflowImage {
	static accept = "";	getBounds() {
		// Calculate the min max bounds for both nodes and groups
		...
		if (app.graph._nodes) {
			...
			app.graph._nodes.forEach((n, i) => {
				...
				DebugLogger.log(`Node ${i}: ${n.type || 'unknown'} at [${n.pos[0]}, ${n.pos[1]}] size [${nodeBounds[2]}, ${nodeBounds[3]}] bounds [${n.pos[0]}-${r}, ${n.pos[1]}-${b}]`);
			});
			
			bounds = app.graph._nodes.reduce(
				(p, n) => {
		...
		if (app.graph._groups) {
			...
			app.graph._groups.forEach((g, i) => {
				...
				DebugLogger.log(`Group ${i}: "${g.title}" at [${g.pos[0]}, ${g.pos[1]}] size [${g.size[0]}, ${g.size[1]}] bounds [${g.pos[0]}-${r}, ${g.pos[1]}-${b}]`);
			});
			
			DebugLogger.log("=== GROUP ANALYSIS ===");
			app.graph._groups.forEach((g, i) => {
				const failingGroups = [0, 3, 5, 7, 9, 11, 13, 15];
				const isExpectedToFail = failingGroups.includes(i);
				DebugLogger.log(`Group ${i}: "${g.title}" - Expected to fail: ${isExpectedToFail}`);
				DebugLogger.log(`  - Color: ${g.color || 'default'}`);
				DebugLogger.log(`  - Font: ${g.font || 'default'}`);
				DebugLogger.log(`  - Visible: ${g.visible !== false}`);
				DebugLogger.log(`  - Properties: ${JSON.stringify(Object.keys(g))}`);
			});
			
			bounds = app.graph._groups.reduce(
				(p, g) => {
	...
	restoreState() {
		...
		// For export, we want full resolution - don't scale down with devicePixelRatio
		const scale = 1; // Always use scale 1 for export to ensure all content fits
		DebugLogger.log(`updateView called with bounds: [${bounds.join(', ')}]`);
		DebugLogger.log(`devicePixelRatio: ${window.devicePixelRatio}, using scale: ${scale} (forced to 1 for export)`);
		
		const width = (bounds[2] - bounds[0]) * scale;
		const height = (bounds[3] - bounds[1]) * scale;
		DebugLogger.log(`Calculated canvas dimensions: ${width} x ${height}`);
		...
		DebugLogger.log(`Canvas set to: ${app.canvas.canvas.width} x ${app.canvas.canvas.height}`);
		DebugLogger.log(`Offset set to: [${app.canvas.ds.offset.join(', ')}]`);
	}
	...
	getDrawTextConfig(_, widget) {
		...
		DebugLogger.clear(); // Clear previous logs
		DebugLogger.log("=== STARTING EXPORT ===");
		// Save the current state of the canvas
		this.saveState();
		DebugLogger.log(`State saved, original canvas size: ${this.state.width} x ${this.state.height}`);
		
		// Update to render the whole workflow
		const bounds = this.getBounds();
		this.updateView(bounds);
		DebugLogger.log(`View updated, new canvas size: ${app.canvas.canvas.width} x ${app.canvas.canvas.height}`);
		DebugLogger.log(`Canvas offset: [${app.canvas.ds.offset.join(', ')}]`);
		DebugLogger.log(`Canvas scale: ${app.canvas.ds.scale}`);
		// Flag that we are saving and render the canvas
		getDrawTextConfig = this.getDrawTextConfig;
		
		DebugLogger.log("About to call app.canvas.draw()");
		app.canvas.draw(true, true);
		DebugLogger.log("app.canvas.draw() completed");
		...
		// Save debug log before downloading the image
